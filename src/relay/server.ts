...
import { BlindRelayError, BlindRelayRoom, type RelayConnection, type RelayRole } from './blind-room.js'
...
export async function startBlindRelayServer(
  options: BlindRelayServerOptions = {},
): Promise<RunningBlindRelayServer> {
  ...
  const touch = (roomId: string, active: ActiveRoom) => {
    clearTimeout(active.timer)
    active.timer = setTimeout(() => { expire(roomId) }, roomIdleMs)
    active.timer.unref?.()
  }
  const roomFor = (roomId: string): ActiveRoom => {
    const existing = rooms.get(roomId)
    if (existing !== undefined) {
      touch(roomId, existing)
      return existing
    }
    if (rooms.size >= maxRooms) throw new Error('relay capacity reached')
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    try {
      ...
      if (url.pathname !== '/v1/connect') throw new Error('unknown upgrade path')
      const roomId = url.searchParams.get('roomId')
      const role = url.searchParams.get('role')
      if (roomId === null || (role !== 'host' && role !== 'mobile')) throw new Error('invalid route')
      decodeBase64Url(roomId, 16)
      if (role === 'mobile' && options.publicOrigin !== undefined
        && req.headers.origin !== options.publicOrigin) throw new Error('origin rejected')
